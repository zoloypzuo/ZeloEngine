...
function Agent_Update(agent, deltaTimeInMillis)
    ...
    local forceToCombine = Agent.ForceToCombine(agent, 100, 180, { leader });

    -- Force to stay away from other agents that are closer than
    -- 2 meters and have less than 180 degree difference in forward
    -- direction.
    local forceToSeparate = Agent.ForceToSeparate(agent, 2, 180, sandboxAgents);
    ...
    local forceToAlign = Agent.ForceToSeparate(agent, 5, 45, { leader });
