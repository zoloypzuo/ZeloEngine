...
namespace
{
	Ogre::Quaternion BtQuaterionToQuaternion(const btQuaternion& quaternion)
	{
		return Ogre::Quaternion(
			quaternion.w(), quaternion.x(), quaternion.y(), quaternion.z());
	}

	btQuaternion QuaternionToBtQuaternion(
		const Ogre::Quaternion& quaternion)
	{
		return btQuaternion(quaternion.x, quaternion.y, quaternion.z, quaternion.w);
	}

	OpenSteer::Vec3 Vector3ToVec3(const Ogre::Vector3& vector)
	{
	...
	Ogre::Vector3 Vec3ToVector3(const OpenSteer::Vec3& vector)
	{
		return Ogre::Vector3(
			Ogre::Real(vector.x), Ogre::Real(vector.y), Ogre::Real(vector.z));
	}

	btVector3 Vector3ToBtVector3(const Ogre::Vector3& vector)
	{
		return btVector3(
...
lua_State* Agent::GetLuaVM() const
{
	return luaVM_;
}
...
btRigidBody* Agent::GetRigidBody() const
{
	return rigidBody_;
}

Sandbox* Agent::GetSandbox() const
{
	return sandbox_;
}

Ogre::SceneNode* Agent::GetSceneNode() const
{
	return sceneNode_;
}
...
void Agent::SetForward(const Ogre::Quaternion& orientation) const
{
	if (rigidBody_)
	{
...
void Agent::SetForward(const Ogre::Vector3& forward) const
{
...
void Agent::SetPosition(const Ogre::Vector3 position) const
{
	if (rigidBody_)
	{
