...
local function AnnoyLeader(inst)
    ...
    if inst.canlootchests then
        ...
        if #items > 0 then
            ...
            inst:DoTaskInTime(math.random(15, 30), function()
                inst.canlootchests = true
            end)
            local item = items[math.random(1, #items)]
            local act = BufferedAction(inst, item, ACTIONS.STEAL)
            act.validfn = function()
                return (item.components.inventoryitem and item.components.inventoryitem:IsHeld())
            end
            return act
        end
    end
    ...
    local root = PriorityNode(
            {
                ...
                RunAway(self.inst, "character", RUN_AWAY_DIST, STOP_RUN_AWAY_DIST, function(hunter)
                    return ShouldRunFn(self.inst, hunter)
                end),
                WhileNode(function()
                    return self.inst.components.combat.target == GetPlayer() and self.inst.HasAmmo(self.inst)
                end, "Attack Player",
                        ChaseAndAttack(self.inst, MAX_CHASE_TIME, MAX_CHASE_DIST)),
                --Pick up poop to throw
                WhileNode(function()
                    return self.inst.components.combat.target == GetPlayer() and not self.inst.HasAmmo(self.inst)
                end, "Pick Up Poop",
                        DoAction(self.inst, GetPoop)),
                --Eat/ pick/ harvest foods.
                WhileNode(function()
                    return self.inst.components.combat.target == GetPlayer() or self.inst.components.combat.target == nil
                end, "Should Eat",
                ...
                WhileNode(function()
                    return self.inst.components.combat.target ~= nil and self.inst.components.combat.target ~= GetPlayer()
                ...
                --Following
                WhileNode(function()
                    return self.inst.harassplayer
                end, "Annoy Leader",
                        DoAction(self.inst, AnnoyLeader)),
                Follow(self.inst, function()
                    return self.inst.harassplayer and GetPlayer()
                end, MIN_FOLLOW_DIST, TARGET_FOLLOW_DIST, MAX_FOLLOW_DIST),

                --Doing nothing
                WhileNode(function()
                    return self.inst.harassplayer
                end, "Wander Around Leader",
                        Wander(self.inst, function()
                            if self.inst.harassplayer then
                                return GetPlayer():GetPosition()
                            end
                        end, MAX_FOLLOW_DIST)),
                WhileNode(function()
                    return not self.inst.harassplayer and not self.inst.components.combat.target
                end,
                        "Wander Around Home", Wander(self.inst, function()
