...
function SetUpPlayerCharacterCallbacks(wilson)
    ...
    wilson:ListenForEvent("death", function(inst, data)
        OnPlayerDeath(wilson, data)
    end)
    ...
    wilson:ListenForEvent("builditem", function(inst, data)
        ProfileStatsAdd("build_item_" .. data.item.prefab)
    end)
    wilson:ListenForEvent("buildstructure", function(inst, data)
        ProfileStatsAdd("build_structure_" .. data.item.prefab)
    end)
end
    ...
    if not TheFrontEnd:IsDisplayingError() then
        ...
        if fast then
            ...
            elseif (GetClock().numcycles == 0 and GetClock():GetNormTime() == 0) or Settings.maxwell ~= nil then
                ...
                max.task = max:StartThread(function()
                    max.components.maxwelltalker:DoTalk()
                end)
            ...
            end, showtitle and 3, showtitle and function()
                SetPause(false)
            end)
        end

        if savedata.map.hideminimap ~= nil then
            GetWorld().minimap:DoTaskInTime(0, function(inst)
                inst.MiniMap:ClearRevealedAreas(savedata.map.hideminimap)
            end)
        end
