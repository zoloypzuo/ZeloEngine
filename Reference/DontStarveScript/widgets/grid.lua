    ...
    for c = 1, self.cols do
        for r = 1, self.rows do
            ...
            if item then
                ...
                if self.h_loop then
                    if c == 1 then
                        left = self:GetItemInSlot(self.cols, r)
                    end
                    if c == self.cols then
                        right = self:GetItemInSlot(1, r)
                    end
                end

                if self.v_loop then
                    if r == 1 then
                        up = self:GetItemInSlot(c, self.rows)
                    end
                    if r == self.rows then
                        down = self:GetItemInSlot(c, 1)
                    end
                end
                ...
                if down then
                    item:SetFocusChangeDir(MOVE_DOWN, down)
                end
                if left then
                    item:SetFocusChangeDir(MOVE_LEFT, left)
                end
                if right then
                    item:SetFocusChangeDir(MOVE_RIGHT, right)
                end
            end
        end
    end
