...
local CraftTabs = Class(Widget, function(self, owner, top_root)
    ...
    self.tabs.onopen = function()
        self.owner.SoundEmitter:PlaySound("dontstarve/HUD/craft_open")
    end
    self.tabs.onchange = function()
        self.owner.SoundEmitter:PlaySound("dontstarve/HUD/craft_open")
    end
    self.tabs.onclose = function()
        self.owner.SoundEmitter:PlaySound("dontstarve/HUD/craft_close")
    end
    self.tabs.onhighlight = function()
        self.owner.SoundEmitter:PlaySound("dontstarve/HUD/recipe_ready")
        return .2
    end
    self.tabs.onalthighlight = function()
    end
    self.tabs.onoverlay = function()
    ...
    self.inst:ListenForEvent("unlockrecipe", function(inst, data)
        self:UpdateRecipes()
    end, self.owner)
...
function CraftTabs:OnControl(control, down)
    if CraftTabs._base.OnControl(self, control, down) then
        return true
    end

    if down and self.focus then
        if control == CONTROL_SCROLLBACK then
            if self.controllercraftingopen and TheInput:GetControlIsMouseWheel(control) then
                -- added TheInput:GetControlIsMouseWheel(control) to this check as the LEFT TRIGGER on a control should not scroll through the menus
                if self.controllercrafting.repeat_time <= 0 then
                    ...
                    self.controllercrafting.repeat_time = TheInput:GetControlIsMouseWheel(control)
