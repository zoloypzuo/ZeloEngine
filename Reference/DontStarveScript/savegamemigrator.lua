        ...
        TheFrontEnd:PushScreen(PopupDialogScreen(STRINGS.UI.MAINSCREEN.MIGRATION_TOOL, string.format(STRINGS.UI.MAINSCREEN.MIGRATION_DIALOG_INFO, frombranch),
                {
                    ...
                    { text = STRINGS.UI.MAINSCREEN.NO, cb = function()
                        TheFrontEnd:PopScreen()
                    end } }
        ))
        ent:Remove()
...
function SaveGameMigrator:DoMigrate()
    ...
    if slotsAvailable < slotsNeeded then
        ...
        TheFrontEnd:PushScreen(PopupDialogScreen(STRINGS.UI.MAINSCREEN.WARNING, slotsNeededString, { { text = STRINGS.UI.MAINSCREEN.OK, cb = function()
            TheFrontEnd:PopScreen()
        end } }))
        return
    else
        ...
        TheFrontEnd:PushScreen(PopupDialogScreen(STRINGS.UI.MAINSCREEN.MIGRATION_SUCCESSFUL_HEADER, STRINGS.UI.MAINSCREEN.MIGRATION_SUCCESSFUL, { { text = STRINGS.UI.MAINSCREEN.OK, cb = function()
            TheFrontEnd:PopScreen()
        end } }))
    end
end
            ...
            if success and string.len(str) > 0 and savedata ~= nil then
                ...
                if data.slots[fromslot] then
                    ...
                    SaveGameIndex:Save(function()
                        print(string.format("Slot %s migrated from %s to slot %d", tostring(fromslot), self.frombranch, tostring(toslot)))
                    end)
                end
            else
                print("Data corrupt for " .. saveIndexBackupName)
