...
local function fn(Sim)
    ...
    inst:ListenForEvent("startrowing", function()
        UpdateSound(inst)
    end)
    inst:ListenForEvent("stoprowing", function()
        UpdateSound(inst)
    end)
...
local function fadein(inst)
    ...
    if inst:IsAsleep() then
        ...
        inst.components.fader:Fade(0, INTENSITY, 0.6, function(v)
...
local function fadeout(inst)
    ...
    if inst:IsAsleep() then
        ...
        inst.components.fader:Fade(INTENSITY, 0, 0.6, function(v)
            inst.Light:SetIntensity(v)
        end, function()
