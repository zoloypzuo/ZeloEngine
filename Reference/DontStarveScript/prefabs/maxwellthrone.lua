...
local function SpawnNewPuppet(inst)
    ...
    if puppet.components.maxwelltalker then
...
local function PlayerDie(inst)
    ...
    inst:DoTaskInTime(24 * (1 / 30), function()
        inst.SoundEmitter:PlaySound("dontstarve/wilson/death")
    end)
    inst:DoTaskInTime(40 * (1 / 30), function()
        inst.SoundEmitter:KillSound("deathrattle")
    end)
    Sleep(4)
    inst:StartThread(function()
...
local function fn(Sim)
    ...
    if characterinthrone == "waxwell" then
        --special case for maxwell

        inst.isMaxwell = true
        characterinthrone = "maxwellendgame"
        inst:ListenForEvent("free", function()
            EndGameSequence(inst)
        end, inst.lock)
        ...
        inst:ListenForEvent("free", function()
            EndGameSequenceNoMaxwell(inst)
        end, inst.lock)

    end

    inst:DoTaskInTime(0, function()
        inst.puppet = SpawnPuppet(inst, characterinthrone)
    end)
