...
function TeamAttacker:OnUpdate(dt)
    if self:ShouldGoHome() then
        self:LeaveTeam()
    end

    if self.teamleader and self.teamleader:CanAttack() then
        --did you find a team?
        if self.orders == "HOLD" or self.orders == nil then
            --if you don't have anything to do.. look menacing
            ...
            if self.formationpos then
                ...
                if destpos and mypos then
                    if distsq(destpos, mypos) >= 0.15 then
                        --if you're almost at your target just stop.
        ...
        elseif self.orders == "ATTACK" then
            --You have been told to attack. Get the target from your leader.
