local Hiddendanger = Class(function(self, inst)
    ...
    GetPlayer():ListenForEvent("equip", function()
        self:TestShouldShowDanger()
    end)
    GetPlayer():ListenForEvent("unequip", function()
        self:TestShouldShowDanger()
    end)
    self.inst:ListenForEvent("entitywake", function()
        self:TestShouldShowDanger()
    end)
end)
...
function Hiddendanger:ShowDanger(state)
    if state then
        if not self.fx and self.inst:IsValid() then
            ...
            self.fx:ListenForEvent("onremove", function()
