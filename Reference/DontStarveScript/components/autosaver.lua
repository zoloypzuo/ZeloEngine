...
function Autosaver:DoSave()
    ...
    if enabled then
        ...
        self.inst:DoTaskInTime(3, function()
            GetPlayer().HUD.controls.saving:EndSave()
        end)
    end
...
function Autosaver:OnUpdate(dt)
    ...
    if self.timeout <= 0 and GetClock():GetTimeInEra() > 10 then
        if self.inst.LightWatcher:IsInLight() and self.inst.components.health:GetPercent() > .1 then
            local danger = FindEntity(self.inst, 30, function(target)
                return target:HasTag("monster") or target.components.combat and target.components.combat.target == self.inst
            end)
            if not danger then
                self:DoSave()
            else
