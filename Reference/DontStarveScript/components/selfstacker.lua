...
function SelfStacker:FindItemToStackWith()
    self.stackpartner = FindEntity(self.inst, self.searchradius, function(item)
        return item.prefab == self.inst.prefab and item.components.selfstacker:CanSelfStack()
    end, { "selfstacker" })
