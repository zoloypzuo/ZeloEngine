...
function Overseer:GaveUp(attacker)
    ...
    if prefab == "penguin" then
        return
    end   -- fix until I figure out why pengulls repeatedly target and give up
    if prefab == "NIL" then
...
function Overseer:InitFight()
    ...
    if player.components.health:IsDead() then
        -- don't start another record if the player is dead
        self.engaged = false
        return
    end
...
function Overseer:Targeted(attacker)
    ...
    if not attacker or attacker == player or not player or
            ...
            (attacker.Transform and player:GetDistanceSqToInst(attacker) >= (15 * 15)) then
        -- some target from a long distance
        return
    end
    ...
    if attacker then
        -- attacker is nil if it's charlie
        data.foeList[attacker] = true
    end

    if prefab ~= "penguin" then
        -- fix until I figure out why penguins repeately target and giveup
        --dprint("\nOS Targeted:",prefab)
...
function Overseer:OnUpdate(dt)
    ...
    local function CheckAura(inst)
        -- is this thing (ghost) hurting the player with an aura?
    ...
    local function CheckTeam(inst)
        -- is this thing in a team that is still targeting the player?
