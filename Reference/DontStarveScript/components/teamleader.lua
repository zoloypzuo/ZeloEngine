...
function TeamLeader:IsTeamFull()
    if self.team and getteamsize(self.team) >= self.max_team_size then
        return true
    end
end
...
function TeamLeader:NewTeammate(member)
    ...
    if self:ValidMember(member) then
        member.deathfn = function()
            self:OnLostTeammate(member)
        end
        member.attackedfn = function()
            self:BroadcastDistress(member)
        end
...
function TeamLeader:OnUpdate(dt)
    ...
    if self.threat and self.threat:IsValid() and self:CanAttack() then
        ...
        if self:AllInState("HOLD") then
            ...
            if self.timebetweenattacks <= 0 then
                ...
                self.inst:DoTaskInTime(0.5, function()
                    self:GiveOrdersToAllWithOrder("ATTACK", "WARN")
                end)
            else
            end
        end
