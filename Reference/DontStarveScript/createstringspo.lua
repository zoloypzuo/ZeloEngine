...
local function PrintStringTableV2(base, tbl, file)
    ...
    if file then
        table.sort(output_strings, function(a, b)
...
local function LookupIdValue(lkp_var, path)
    ...
    if endnum2 then
        evalstr = evalstr .. "[" .. endnum2 .. "]"
    end
    if endnum1 then
        evalstr = evalstr .. "[" .. endnum1 .. "]"
    end
    local result, val = pcall(function()
        return loadstring(evalstr)()
    end)
