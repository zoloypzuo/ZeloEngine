    ...
    while GetTableSize(unusedTasks) > 0 do
        ...
        for taskid, node in pairs(unusedTasks) do
            ...
            for i, v in ipairs(self.tasks[taskid].locks) do
                ...
                locks[v] = lock
                print_lockandkey_ex("\t\tLock:", LOCKS_ARRAY[v], tabletoliststring(lock.keys, function(x)
                    return KEYS_ARRAY[x]
                end))
            end
            ...
            for lock, lockData in pairs(locks) do
                ...
                for key, keyNodes in pairs(availableKeys) do
                    ...
                    for reqKeyIdx, reqKey in ipairs(lockData.keys) do
                        -- this lock?
                        if reqKey == key then
                            -- If yes, get the nodes with
                            -- that key so that we
                            for i, node in ipairs(keyNodes) do
                                -- can potentially attach
...
function Story:GetRandomNodeFromTasks(taskSet)
    ...
    if sz > 0 then
        ...
        for taskid, _ in pairs(taskSet) do
            -- special order
            task = taskid
            if choice <= 0 then
                break
