...
function Graph:SaveEncode(map, encoded)
    ...
    for id, node in pairs(self.exit_nodes) do
        --self.nodes) do
...
function Graph:ProcessInsanityWormholes(entities, width, height)
    ...
    local IsNodeAWormhole = function(node)
        return IsNodeTagged(node, "OneshotWormhole")
    end
    ...
    local DoWormholeLayout = function(node, data)
        ...
        local add_fn = { fn = function(...)
            node:AddEntity(...)
        end, args = { entitiesOut = entities, width = width, height = height, rand_offset = false, debug_prefab_list = prefab_list } }
...
function Graph:ApplyPoisonTag()
    ...
    for k, node in pairs(nodes) do
        ...
        if IsNodeTagged(node, "ForceDisconnected") then
            --or string.find(node.id, "LOOP_BLANK_SUB")~=nil then
