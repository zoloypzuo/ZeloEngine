...
function FindFlower:Visit()
    ...
    if self.status == RUNNING then
        if not self.inst.components.pollinator.target
            ...
            return guy ~= self.inst and guy.components.pollinator and guy.components.pollinator.target == self.inst.components.pollinator.target
        end, { "pollinator" }) then
            self.status = FAILED
        end
    end
...
function FindFlower:PickTarget()
    ...
    if closestFlower
            ...
            and not FindEntity(closestFlower, 2, function(guy)
        return guy.components.pollinator and guy.components.pollinator.target == closestFlower
    end, { "pollinator" }) then
