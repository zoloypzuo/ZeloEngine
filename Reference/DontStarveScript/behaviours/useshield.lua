UseShield = Class(BehaviourNode, function(self, inst, damageforshield, shieldtime, hidefromprojectiles)
    ...
    self.inst:ListenForEvent("hostileprojectile", function()
        self:OnAttacked(nil, 0, true)
    end)
    self.inst:ListenForEvent("firedamage", function()
        self:OnAttacked()
    end)
    self.inst:ListenForEvent("startfiredamage", function()
        self:OnAttacked()
    end)
end)
