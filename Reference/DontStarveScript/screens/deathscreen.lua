...
function DeathScreen:NotCapped(days_survived, start_xp, escaped)
    ...
    local menu_items = {
        { text = STRINGS.UI.DEATHSCREEN.MAINMENU, cb = function()
            self:OnMenu(escaped)
        end }
    }

    if escaped then
        table.insert(menu_items, { text = STRINGS.UI.DEATHSCREEN.CONTINUE, cb = function()
            self:OnContinue()
        end })
    else
        table.insert(menu_items, { text = STRINGS.UI.DEATHSCREEN.RETRY, cb = function()
            self:OnRetry()
        end })
    end
