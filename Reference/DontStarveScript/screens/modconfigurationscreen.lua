...
local ModConfigurationScreen = Class(Screen, function(self, modname)
    ...
    self.applybutton = self.menu:AddItem(STRINGS.UI.MODSSCREEN.APPLY, function()
        self:Apply()
    end, Vector3(-260, -290, 0))
    self.cancelbutton = self.menu:AddItem(STRINGS.UI.MODSSCREEN.CANCEL, function()
        self:Cancel()
    end, Vector3(-110, -290, 0))
    self.resetbutton = self.menu:AddItem(STRINGS.UI.MODSSCREEN.RESETDEFAULT, function()
        self:ResetToDefaultValues()
    end, Vector3(205, -290, 0))
    ...
    if #options <= ROWS_PER_COL * COLS then
        -- Only show the arrow if we have a ton of options
...
function ModConfigurationScreen:OnControl(control, down)
    if ModConfigurationScreen._base.OnControl(self, control, down) then
        return true
    end
    ...
    for k = 1, ROWS_PER_COL * 2 do
        ...
        if options[idx] then
            ...
            if default_value == nil then
                default_value = options[idx].default
            end
