...
function SlotDetailsScreen:BuildMenu()
    ...
    if backup_index ~= nil and BRANCH == "dev" then
        ...
        menuitems = {
            { name = STRINGS.UI.SLOTDETAILSSCREEN.CONTINUE, fn = function()
                self:Continue()
            end, offset = Vector3(0, 20, 0) },
            { name = STRINGS.UI.SLOTDETAILSSCREEN.DELETE, fn = function()
                self:Delete()
            end },
            { name = STRINGS.UI.SLOTDETAILSSCREEN.CANCEL, fn = function()
                EnableAllDLC()
                TheFrontEnd:PopScreen(self)
            end },
            { name = STRINGS.UI.SAVEINTEGRATION.RESTORE_BUTTON, fn = function()
                self:RestoreSave()
            end },
        }
    else
    ...
    if self.RoG and not IsDLCInstalled(REIGN_OF_GIANTS) then
        for i, j in pairs(self.menu.items) do
            if j:GetText() == STRINGS.UI.SLOTDETAILSSCREEN.CONTINUE then
                ...
                j:SetOnClick(function()
                    self:PushCantContinueDialog(REIGN_OF_GIANTS)
                end)
            end
        end
    end
    if self.capyDLC and not IsDLCInstalled(CAPY_DLC) then
        for i, j in pairs(self.menu.items) do
            if j:GetText() == STRINGS.UI.SLOTDETAILSSCREEN.CONTINUE then
                ...
                j:SetTextFocusColour(1, 0, 0, .75)
                j:SetOnClick(function()
                    self:PushCantContinueDialog(CAPY_DLC)
                end)
            end
        end
    end
    ...
    if self.porkDLC and not IsDLCInstalled(PORKLAND_DLC) then
        for i, j in pairs(self.menu.items) do
            if j:GetText() == STRINGS.UI.SLOTDETAILSSCREEN.CONTINUE then
                ...
                j:SetTextFocusColour(1, 0, 0, .75)
                j:SetOnClick(function()
                    self:PushCantContinueDialog(PORKLAND_DLC)
                end)
            end
        end
    end
...
function SlotDetailsScreen:OnControl(control, down)
    if SlotDetailsScreen._base.OnControl(self, control, down) then
        return true
    end
    ...
    local menu_items = {
        -- ENTER
        {
            ...
            cb = function()
                EnableAllDLC()
                TheFrontEnd:PopScreen()
                SaveGameIndex:DeleteSlot(self.saveslot, function()
                    TheFrontEnd:PopScreen()
                end)
            end
        },
...
function SlotDetailsScreen:PushCantContinueDialog(index)
    local menu_items = {
        -- OK
        { text = STRINGS.UI.MAINSCREEN.OK, cb = function()
            TheFrontEnd:PopScreen()
            self.menu:SetFocus()
        end },
    }
...
function SlotDetailsScreen:ShowModalModsDisabledWarning(disabledmods)
    ...
    for i, v in ipairs(disabledmods) do
        ...
        if i == 1 then
            -- No comma for first mod in the list
            message_body = message_body .. name
        elseif string.len(message_body .. ", " .. name) <= maxlistlength then
            -- Subsequent mods get a comma and added, but only if they don't break max size
            message_body = message_body .. ", " .. name
        else
            truncated = true
            break
