...
local BroadcastingOptionsScreen = Class(Screen, function(self, in_game)
    ...
    help_button:SetOnClick(function()
        VisitURL("https://kleisupport.desk.com/customer/portal/articles/1536861-how-to-enable-twitch-broadcasting-and-chat")
    end)
...
function BroadcastingOptionsScreen:OnControl(control, down)
    if BroadcastingOptionsScreen._base.OnControl(self, control, down) then
        return true
    end
...
function BroadcastingOptionsScreen:UpdateMenu()
    ...
    if self:IsDirty() then
        ...
        self.menu:AddItem(STRINGS.UI.BROADCASTING.APPLY, function()
            self:ApplyChanges()
        end, Vector3(50, 30, 0))
        self.menu:AddItem(STRINGS.UI.BROADCASTING.REVERT, function()
            self:RevertChanges()
        end, Vector3(-50, 30, 0))
    else
        self.menu.horizontal = true
        self.menu:AddItem(STRINGS.UI.BROADCASTING.CLOSE, function()
            self:Accept()
        end, Vector3(50, 30, 0))
        if broadcasting_options:GetBroadcastingEnabled() then
            if broadcasting_options:IsPastInitializedState() then
                self.menu:AddItem(STRINGS.UI.BROADCASTING.LOGOUT, function()
                    self:Logout()
                end, Vector3(-50, 30, 0))
            else
                self.menu:AddItem(STRINGS.UI.BROADCASTING.LOGIN, function()
                    self:Login()
                end, Vector3(-50, 30, 0))
            end
        end
    ...
    if broadcasting_options:IsPastInitializedState() and broadcasting_options:GetBroadcastingEnabled() then
        ...
        if broadcasting_url ~= nil and broadcasting_url ~= "" then
            ...
            if broadcasting_options:IsStreaming() then
                self.menu:AddItem(STRINGS.UI.BROADCASTING.STOPSTREAM, function()
                    self:ToggleStreaming()
                end, Vector3(30, 110, 0))
            else
                self.menu:AddItem(STRINGS.UI.BROADCASTING.STARTSTREAM, function()
                    self:ToggleStreaming()
                end, Vector3(30, 110, 0))
            end

            self.menu:AddItem(STRINGS.UI.BROADCASTING.VIEWSTREAM, function()
                VisitURL(broadcasting_url)
            end, Vector3(110, 190, 0))
        end
    end
...
function BroadcastingOptionsScreen:DoInit()
    ...
    self.enabledSpinner.OnChanged = function(_, data)
        this.working.broadcasting = data
        self:UpdateMenu()
    end

    self.frameRateSpinner = NumericSpinner(10, 30)
    self.frameRateSpinner.OnChanged = function(_, data)
    ...
    self.bandwidthSpinner.OnChanged = function(_, data)
