...
function BroadcastingLoginScreen:OnControl(control, down)
    if BroadcastingLoginScreen._base.OnControl(self, control, down) then
        return true
    end
...
function BroadcastingLoginScreen:DoInit()
    ...
    local menu_items = {
        { text = STRINGS.UI.BROADCASTING.LOGIN, cb = function()
            self:Login()
        end },
        { text = STRINGS.UI.BROADCASTING.SIGNUP, cb = function()
            VisitURL("http://twitch.tv")
        end },
        { text = STRINGS.UI.BROADCASTING.CANCEL, cb = function()
            self:Close()
        end },
    }
