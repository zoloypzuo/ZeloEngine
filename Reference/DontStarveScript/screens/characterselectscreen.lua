...
local CharacterSelectScreen = Class(Screen, function(self, profile, cb, no_backbutton, default_character, rog_enabled)
    ...
    for k = 1, 3 do
        ...
        character_portrait.OnControl = function(_, control, down)
            if control == CONTROL_ACCEPT then
                if not down then
                    self:OnClickPortait(k)
                    return true
                end
            end
        end
        character_portrait.OnGainFocus = function()
            if self.portrait_idx ~= k then
                portrait_bg:GetAnimState():PlayAnimation("mouseover")
            end
        end
        character_portrait.OnLoseFocus = function()
            if self.portrait_idx ~= k then
                portrait_bg:GetAnimState():PlayAnimation("idle")
            end
        end
...
function CharacterSelectScreen:OnControl(control, down)
    ...
    if CharacterSelectScreen._base.OnControl(self, control, down) then
        return true
    end
