...
local NewGameScreen = Class(Screen, function(self, slotnum)
    ...
    local start_btn = { text = STRINGS.UI.NEWGAMESCREEN.START, cb = function()
        self:Start()
    end }
    local char_btn = { text = STRINGS.UI.NEWGAMESCREEN.CHANGECHARACTER, cb = function()
        self:ChangeCharacter()
    end }
    local world_btn = { text = STRINGS.UI.NEWGAMESCREEN.CUSTOMIZE, cb = function()
        self:Customize()
    end }
    local cancel_btn = {
    ...
    if IsDLCInstalled(REIGN_OF_GIANTS) or IsDLCInstalled(CAPY_DLC) or IsDLCInstalled(PORKLAND_DLC) then
        ...
        if IsDLCInstalled(REIGN_OF_GIANTS) then
            table.insert(self.dlc_buttons, self:MakeReignOfGiantsButton())
        end
        if IsDLCInstalled(CAPY_DLC) then
            table.insert(self.dlc_buttons, self:MakeCapyButton())
        end
        if IsDLCInstalled(PORKLAND_DLC) then
            table.insert(self.dlc_buttons, self:MakePorklandButton())
        end
        ...
        for i, v in pairs(self.customoptions.faketweak) do
            for m, n in pairs(self.customoptions.tweak) do
                for j, k in pairs(n) do
                    if v == j then
                        -- Found the fake tweak setting, now remove it from the table
    ...
    local function CleanupTweakTable()
        ...
        if self.customoptions and self.customoptions.faketweak and self.customoptions.tweak and #self.customoptions.faketweak > 0 then
            for i, v in pairs(self.customoptions.faketweak) do
                for m, n in pairs(self.customoptions.tweak) do
                    for j, k in pairs(n) do
                        if v == j and k == "default" then
                            -- Found the fake tweak setting for "default", now remove it from the table
                            self.customoptions.tweak[m][j] = nil
                            break
                        end
                    end
