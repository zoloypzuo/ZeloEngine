...
function DebugMenuScreen:OnBecomeActive()
    ...
    for k, v in pairs(valid_recipes) do
        ...
        table.insert(craft_menus[v.tab], menus.DoAction(v.name, function()
            for kk, vv in pairs(v.ingredients) do
                c_give(vv.type, vv.amount)
            end
        end))
    end
    ...
    local bars = {
        menus.NumericToggle("Health", 1, 100, function()
            return math.floor(GetPlayer().components.health:GetPercent() * 100)
        end, function(val)
            GetPlayer().components.health:SetPercent(val / 100)
        end, 5),
        menus.NumericToggle("Sanity", 1, 100, function()
            return math.floor(GetPlayer().components.sanity:GetPercent() * 100)
        end, function(val)
            GetPlayer().components.sanity:SetPercent(val / 100)
        end, 5),
        menus.NumericToggle("Hunger", 1, 100, function()
            return math.floor(GetPlayer().components.hunger:GetPercent() * 100)
        end, function(val)
            GetPlayer().components.hunger:SetPercent(val / 100)
        end, 5)
    }
    local timecontrol = {
        menus.NumericToggle("Adjust Time Warp", 0, 4, function()
            return time_warp
        end, function(val)
            time_warp = val
        end, .25),
        menus.DoAction("Advance Day", function()
            LongUpdate(TUNING.TOTAL_DAY_TIME, true)
        end),
        menus.DoAction("Advance Segment", function()
            LongUpdate(TUNING.TOTAL_DAY_TIME / 16, true)
        end)
    }
    local teleport = {
        menus.DoAction("Eyebone", function()
            c_gonext("chester_eyebone")
            self:Close()
        end),
        menus.DoAction("Maxwell Door", function()
            c_gonext("adventure_portal")
            self:Close()
        end),
        menus.DoAction("Cave Entrance", function()
            c_gonext("cave_entrance")
            self:Close()
        end),
        menus.DoAction("Cave Exit", function()
            c_gonext("cave_exit")
            self:Close()
        end),
    }
    ...
    for k, v in pairs(Prefabs) do
        ...
        if v.name == "forest" or v.name == "cave" then
            can_spawn = false
        end
        if can_spawn then
    ...
    for k, v in pairs(spawn_lists) do
        ...
        for kk, vv in pairs(v) do
            table.insert(inner_list, menus.DoAction(vv, function()
                DebugSpawn(vv).Transform:SetPosition(GetPlayer().Transform:GetWorldPosition())
            end))
        end
    ...
    local weathercontrol = {
        menus.CheckBox("Toggle Precipitation", function()
            return GetSeasonManager().precip
        end,
                function(val)
                    if val then
                        GetSeasonManager().atmo_moisture = GetSeasonManager().moisture_limit * 2
                        GetSeasonManager():OnUpdate(0)
                    else
                        GetSeasonManager().atmo_moisture = 0
                        GetSeasonManager():OnUpdate(0)
                    end
                end),
        menus.CheckBox("Toggle Winter", function()
    ...
    if InGamePlay() then
        table.insert(main_options, menus.CheckBox("Toggle God Mode", function()
            return god
        end, function(val)
            god = val
            GetPlayer().components.health:SetInvincible(god)
        end))
        table.insert(main_options, menus.CheckBox("Toggle Reveal Map", function()
            return map_reveal
        end, function(val)
            map_reveal = val
            map.MiniMap:EnableFogOfWar(not map_reveal)
        end))
        table.insert(main_options, menus.CheckBox("Toggle Free Crafting", function()
            return free_craft
        end, function(val)
            free_craft = val
            GetPlayer().components.builder:GiveAllRecipes()
            GetPlayer():PushEvent("techlevelchange")
        end))
        table.insert(main_options, menus.DoAction("Run Profiler", function()
            self:Close()
            self:RunProfiler()
        end))
        ...
        local adv_slot = 1
        local adventure_mode_menu = { menus.NumericToggle("Overwrite Slot:", 1, 4, function()
            return adv_slot
        end, function(v)
            adv_slot = v
        end) }
        ...
        table.insert(main_options, menus.DoAction("Unlock All Characters", function()
            Profile:UnlockEverything()
            self:Close()
        end))
        table.insert(main_options, menus.DoAction("Reset Profile", function()
            Profile:Reset()
            self:Close()
        end))
    ...
    table.insert(main_options, menus.DoAction("Restart", function()
        StartNextInstance()
        self:Close()
    end))
...
function DebugMenuScreen:RunProfiler()
    ...
    TheFrontEnd:PushScreen(RunningProfilePopup(RECORD_SECONDS, function()
        --[[pass]]
    end))
end

function DebugMenuScreen:OnControl(control, down)
    if DebugMenuScreen._base.OnControl(self, control, down) then
        return true
    end
