    ...
    for line in JoinPOFileMultiline(resolvefilepath(fname)) do
        ...
        if not newformat_flag then
            if string.find(line, "POT Version: 2.0", 0, true)
                    or string.find(line, "X-Generator: Poedit", 0, true) then
...
function Translator:GetTranslatedString(strid, lang)
    ...
    if lang and self.languages[lang] then
        if self.dbfile then
            self.dbfile:write("Reqested id: " .. strid .. "\t\t\t" .. tostring(self.languages[lang][strid]) .. "\n")
        end
...
function TranslateStringTable(tbl)
    ...
    if LanguageTranslator.dbfile then
        LanguageTranslator.dbfile:write("Translator: Translating string table....\n")
    end
    ...
    if LanguageTranslator.dbfile then
        LanguageTranslator.dbfile:close()
    end
