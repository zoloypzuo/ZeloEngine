...
FrontEnd = Class(function(self, name)
    ...
    TheInput:AddTextInputHandler(function(text)
...
function FrontEnd:OnControl(control, down, isrepeat)
    ...
    if #self.screenstack > 0 then
        ...
        end  --map this one for buttons
        if screen:OnControl(control, down) then
            return true
        end
    end
...
function FrontEnd:Update(dt)
    ...
    elseif TheInput:IsControlPressed(CONTROL_SCROLLBACK) then
        local repeat_time = TheInput:GetControlIsMouseWheel(CONTROL_SCROLLBACK) and
        ...
        --if TheInput:IsControlPressed(CONTROL_SCROLLFWD) then
        local repeat_time = TheInput:GetControlIsMouseWheel(CONTROL_SCROLLFWD) and
                MOUSE_SCROLL_REPEAT_TIME or
                SCROLL_REPEAT_TIME
        if self.scroll_repeat_time < 0 then
            self.scroll_repeat_time = repeat_time > dt and repeat_time - dt or 0
...
function FrontEnd:OnMouseButton(button, down, x, y)
    ...
    if #self.screenstack > 0 then
        if self.screenstack[#self.screenstack]:OnMouseButton(button, down, x, y) then
            return true
        end
    end
