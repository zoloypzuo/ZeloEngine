    ...
    if ret then
        for i, val in ipairs(ret) do
            if type(val) == "table" then
                --and val.is_a and val:is_a(Achievements)then
...
function CheckControllers()
    ...
    if isConnected and not sawPopup then
        ...
        local function enableControllers()
            ...
            Profile:Save()
            scheduler:ExecuteInTime(0.05, function()
                Check_Mods()
            end)
        end
...
function StartNextInstance(in_params, send_stats)
    ...
    if LOADED_CHARACTER then
        ...
        if player and player.components.vision then
            -- and stops trying to update vision
...
function RequestShutdown()
    ...
    if string.len(stats) <= 12 then
        -- empty stats are '{"stats":[]}'
        Shutdown()
        return
    end
                ...
                ScriptErrorScreen(
                        ...
                        error,
                        {
                            ...
                            { text = STRINGS.UI.MAINSCREEN.MODFORUMS, nopop = true, cb = function()
                                VisitURL("http://forums.kleientertainment.com/index.php?/forum/26-dont-starve-mods-and-tools/")
                            end },
                            ENABLE_BUG_REPORTER and { text = STRINGS.UI.MAINSCREEN.BUGREPORT, cb = function()
                                TheFrontEnd:PushScreen(BugReportScreen(), true)
                            end } or nil
                        },
                        ANCHOR_LEFT,
        ...
        TheFrontEnd:DisplayError(
                ScriptErrorScreen(
                        ...
                        error,
                        {
                            ...
                            { text = STRINGS.UI.MAINSCREEN.FORUM, nopop = true, cb = function()
                                VisitURL("http://forums.kleientertainment.com/index.php?/forum/26-dont-starve-mods-and-tools/")
                            end },
                            ENABLE_BUG_REPORTER and { text = STRINGS.UI.MAINSCREEN.BUGREPORT, cb = function()
                                TheFrontEnd:PushScreen(BugReportScreen(), true)
