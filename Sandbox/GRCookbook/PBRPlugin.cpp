...
void Ch6PBRPlugin::loadMesh(const std::string &meshPath) {
    ...
    const size_t kSizeIndices = sizeof(glm::uint32_t) * indices.size();
    const size_t kSizeVertices = sizeof(VertexData) * vertices.size();

    mesh = std::make_unique<GLMeshPVP>(indices.data(), (glm::uint32_t) kSizeIndices,
                                       (float *) vertices.data(), (glm::uint32_t) kSizeVertices);
}
