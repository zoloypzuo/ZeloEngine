...
local function UpdateVisibility(userData)
    ...
    for index = 1, #agents do
        if (agents[index] ~= userData.agent) then
            canSeeAgentInfo = AgentSenses_CanSeeAgent(userData, agents[index], false);
...
function AgentSenses_InitializeSenses(userData)
    ...
    eventCallbacks[AgentCommunications.EventType.BulletImpact] = HandleBulletImpactEvent;
    eventCallbacks[AgentCommunications.EventType.BulletShot] = HandleBulletShotEvent;
    eventCallbacks[AgentCommunications.EventType.DeadEnemySighted] = HandleDeadEnemySightedEvent;
    eventCallbacks[AgentCommunications.EventType.DeadFriendlySighted] = HandleDeadFriendlySightedEvent
    eventCallbacks[AgentCommunications.EventType.EnemySighted] = HandleEnemySightedEvent;
