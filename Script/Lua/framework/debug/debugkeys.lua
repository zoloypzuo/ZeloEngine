...
function AddGameDebugKey(key, fn, down)
    ...
    table.insert(handlers[key], function(_down)
        if _down == down and inGamePlay then
            return fn()
        end
    end)
end
...
AddGameDebugKey(KEY_R, function()
    if TheInput:IsKeyDown(KEY_SHIFT) then
        ...
        if ents[1] and ents[1].prefab then
            ents[1]:Remove()
        end
        return true
    end
...
AddGameDebugKey(KEY_F7, function()
    ...
    for k, v in pairs(ents) do
        ...
        if v and v.components.selfstacker:CanSelfStack() then
            v:DoTaskInTime(math.random() * .1, function()
                v.components.selfstacker:DoStack()
            end)
        end
    end
