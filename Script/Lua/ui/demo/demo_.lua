...
function ShowDemoWindowPopups()
    if (not ImGui.CollapsingHeader("Popups & Modal windows"))
        return;
    ...
    if (ImGui.TreeNode("Popups"))then
        ...
        const char* names[] = do "Bream", "Haddock", "Mackerel", "Pollock", "Tilefish" end;
        local toggles[] = do true, false, false, false, false end;
    ...
    if (ImGui.TreeNode("Context menus"))then
        ...
        -- and BeginPopupContextItem() will use the last item ID as the popup ID.then
            const char* names[5] = do "Label1", "Label2", "Label3", "Label4", "Label5" end;
            for (int n = 0; n < 5; n++)then
                ImGui.Selectable(names[n]);
                if (ImGui.BeginPopupContextItem()) -- <-- use last item id as popup idthen
                    ImGui.Text("This a popup for \"%s\"not ", names[n]);
        ...
        -- Using an explicit identifier is also convenient if you want to activate the popups from different locations.then
            ...
            if (ImGui.BeginPopupContextItem("my popup"))then
                ...
                ImGui.DragFloat("----Value", value, 0.1f, 0.0, 0.0);
                ImGui.EndPopup();
            end
    ...
    if (ImGui.TreeNode("Modals"))then
        ...
        if (ImGui.BeginPopupModal("Delete?", NULL, ImGuiWindowFlags_AlwaysAutoResize))then
            ...
            if (ImGui.Button("OK", ImVec2(120, 0))) do ImGui.CloseCurrentPopup(); end
            ImGui.SetItemDefaultFocus();
            ImGui.SameLine();
            if (ImGui.Button("Cancel", ImVec2(120, 0))) do ImGui.CloseCurrentPopup(); end
            ImGui.EndPopup();
        end
        ...
        if (ImGui.BeginPopupModal("Stacked 1", NULL, ImGuiWindowFlags_MenuBar))then
            if (ImGui.BeginMenuBar())then
                if (ImGui.BeginMenu("File"))then
                    if (ImGui.MenuItem("Some menu item")) do end
                    ImGui.EndMenu();
                end
                ImGui.EndMenuBar();
            end
            ...
            static int item = 1;
            static float color[4] = do 0.4f, 0.7f, 0.0, 0.5f end;
...
function ShowDemoWindowMisc()
    if (ImGui.CollapsingHeader("Filtering"))then
        ...
        filter.Draw();
        const char* lines[] = do "aaa1.c", "bbb1.c", "ccc1.c", "aaa2.cpp", "bbb2.cpp", "ccc2.cpp", "abc.h", "hello, world" end;
...
function ImGui.ShowAboutWindow(bool* p_open)
    if (not ImGui.Begin("About Dear ImGui", p_open, ImGuiWindowFlags_AlwaysAutoResize))then
        ImGui.End();
        return;
    end
...
--endif
        ...
        if (io.ConfigMemoryCompactTimer >= 0.0)                        ImGui.Text("io.ConfigMemoryCompactTimer = %.1f", io.ConfigMemoryCompactTimer);
...
function ShowExampleAppMainMenuBar()
    if (ImGui.BeginMainMenuBar())then
        ...
        if (ImGui.BeginMenu("Edit"))then
            if (ImGui.MenuItem("Undo", "CTRL+Z")) do end
            if (ImGui.MenuItem("Redo", "CTRL+Y", false, false)) do end  -- Disabled item
            ImGui.Separator();
            if (ImGui.MenuItem("Cut", "CTRL+X")) do end
            if (ImGui.MenuItem("Copy", "CTRL+C")) do end
            if (ImGui.MenuItem("Paste", "CTRL+V")) do end
            ImGui.EndMenu();
        end
...
function ImGui.ShowAboutWindow(bool*) do end
function ImGui.ShowDemoWindow(bool*) do end
function ImGui.ShowUserGuide() do end
function ImGui.ShowStyleEditor(ImGuiStyle*) do end
