...
void ClosestPtPointRect(Point p, Point a, Point b, Point c, Point &q) {
    Vector
    ab = b � a; // vector across rect
    Vector
    ac = c � a; // vector down rect
    Vector
    d = p � a;
...
Vector n = Cross(b
� a, c � a);
float rab = Dot(n, Cross(a
� r, b � r)); // proportional to signed area of RAB
float rbc = Dot(n, Cross(b
� r, c � r)); // proportional to signed area of RBC
float rca = Dot(n, Cross(c
� r, a � r)); // proportional to signed area of RCA
...
if (a1 != 0.0f && a2 != 0.0f &&
a1 *a2<
0.0f) ... // for floating-point variables
if ((a1 | a2) != 0 &&
a1 ^a2<
...
int TestSpherePolygon(Sphere s, Polygon p) {
    ...
    Vector
    n = Normalize(Cross(p.v[1] � p.v[0], p.v[2] � p.v[0]));
...
Vector m = Cross(pq, p);
u = Dot(pq, Cross(c, b)) + Dot(m, c
� b);
v = Dot(pq, Cross(a, c)) + Dot(m, a
� c);
w = Dot(pq, Cross(b, a)) + Dot(m, b
� a);
...
p = Plane(n, a);
tri.
edgePlaneBC = Plane(Cross(n, c
� b), b);
tri.
edgePlaneCA = Plane(Cross(n, a
� c), c);

--------------------------------------------------------------------------------

tri.edgePlaneBC *= 1.0f / (
Dot(a, tri
.edgePlaneBC.n) � tri.edgePlaneBC.d);
tri.edgePlaneCA *= 1.0f / (
Dot(b, tri
.edgePlaneCA.n) � tri.edgePlaneCA.d);
