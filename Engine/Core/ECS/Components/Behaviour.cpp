...
{
	LuaCall("OnEnd"); 	LuaCall("OnDestroy");
}
...
{
	LuaCall("OnCollisionStart", otherObject); 	LuaCall("OnCollisionEnter", otherObject);
}
...
{
	LuaCall("OnCollisionStop", otherObject); 	LuaCall("OnCollisionExit", otherObject);
}

void OvCore::ECS::Components::Behaviour::OnTriggerEnter(Components::CPhysicalObject& otherObject)
{
	LuaCall("OnTriggerStart", otherObject); 	LuaCall("OnTriggerEnter", otherObject);
}
...
{
	LuaCall("OnTriggerStop", otherObject); 	LuaCall("OnTriggerExit", otherObject);
}
...
void OvCore::ECS::Components::Behaviour::OnInspector(OvUI::Internal::WidgetContainer & root)
{
	...
	if (m_object.valid())
	{
		...
		root.CreateWidget<OvUI::Widgets::Texts::TextColored>("Your script gets interpreted by the engine with success", OvUI::Types::Color::White);
	}
	else
	{
		root.CreateWidget<OvUI::Widgets::Texts::TextColored>("Compilation failed!", OvUI::Types::Color::Red);
		root.CreateWidget<OvUI::Widgets::Texts::TextColored>("Check the console for more information", OvUI::Types::Color::White);
	}
}
