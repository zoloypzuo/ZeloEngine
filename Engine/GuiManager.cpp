    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (const ImDrawCmd *pcmd = cmd_list->CmdBuffer.begin(); pcmd != cmd_list->CmdBuffer.end(); pcmd++) {
            if (pcmd->UserCallback) {
                ...
                glScissor((int) pcmd->ClipRect.x, (int) (static_cast<float>(fb_height) - pcmd->ClipRect.w),
...
//    io.GetClipboardTextFn = Window::getClipboardText;
    ...
    io.DisplayFramebufferScale = ImVec2(displaySize.x > 0 ? (drawableSize.x / displaySize.x) : 0,
                                        displaySize.y > 0 ? (drawableSize.y / displaySize.y) : 0);
}
...
void GuiManager::tick(std::chrono::microseconds delta) {
    Window *window = Engine::getSingletonPtr()->getWindow();
    ImGuiIO &io = ImGui::GetIO();
        ...
        for (auto &property : component->m_properties) {
            ...
            switch (property.second.type) {
                case PropertyType::FLOAT:
                    ImGui::SliderFloat("##value", (float *) property.second.p, property.second.min,
                                       property.second.max);
                    break;
                case PropertyType::FLOAT3:
                    ImGui::SliderFloat3("##value", (float *) property.second.p, property.second.min,
                                        property.second.max);
                    break;
                ...
                case PropertyType::ANGLE:
                    ImGui::SliderAngle("##value", (float *) property.second.p, property.second.min,
                                       property.second.max);
                    break;
            }
            ImGui::PopStyleColor();
...
void renderSceneGraph(Entity *sceneGraph) {
    ...
    if (node_open) {
        ...
        for (const auto &component : sceneGraph->getComponents()) {
            renderComponent(component.get());
        }

        for (const auto &entity : sceneGraph->getChildren()) {
