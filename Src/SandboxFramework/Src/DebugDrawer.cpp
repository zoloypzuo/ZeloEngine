...
{
	std::vector<LineIndices>::iterator result = std::find(lineIndices.begin(), lineIndices.end(),
	                                                      LineIndices(index0, index1));
...
int IcoSphere::addToVertices(std::vector<VertexPair>* target, const Ogre::Vector3& position,
                             const Ogre::ColourValue& colour, float scale)
{
...
template <>
DebugDrawer* Ogre::Singleton<DebugDrawer>::msSingleton = nullptr;
                              ...
                              float alpha)
{
	...
	{
		addLineVertex(Ogre::Vector3(centre.x + radius * Ogre::Math::Cos(angle), centre.y,
		                            centre.z + radius * Ogre::Math::Sin(angle)),
                                    ...
                                    float alpha)
{
	...
	for (int i = 0; i < segmentsCount; i++)
	{
		addTriangleVertex(Ogre::Vector3(centre.x + radius * Ogre::Math::Cos(angle), centre.y,
		                                centre.z + radius * Ogre::Math::Sin(angle)),
		                  Ogre::ColourValue(colour.r, colour.g, colour.b, alpha));
		angle += increment;
	}
                                ...
                                float alpha)
{
	...
	for (int i = 0; i < segmentsCount; i++)
	{
		addLineVertex(Ogre::Vector3(centre.x + radius * Ogre::Math::Cos(angle), centre.y + height / 2,
		                            centre.z + radius * Ogre::Math::Sin(angle)),
		              Ogre::ColourValue(colour.r, colour.g, colour.b, alpha));
		angle += increment;
	}
	...
	for (int i = 0; i < segmentsCount; i++)
	{
		...
		addLineIndices(segmentsCount + index + i,
		               i + 1 < segmentsCount ? segmentsCount + index + i + 1 : segmentsCount + index);
                                      ...
                                      float alpha)
{
	...
	for (int i = 0; i < segmentsCount; i++)
	{
		addTriangleVertex(Ogre::Vector3(centre.x + radius * Ogre::Math::Cos(angle), centre.y + height / 2,
		                                centre.z + radius * Ogre::Math::Sin(angle)),
		                  Ogre::ColourValue(colour.r, colour.g, colour.b, alpha));
		angle += increment;
	}

	addTriangleVertex(Ogre::Vector3(centre.x, centre.y + height / 2, centre.z),
	                  Ogre::ColourValue(colour.r, colour.g, colour.b, alpha));
                             ...
                             bool isFilled)
{
	...
	if (isFilled)
	{
		...
		trianglesIndex += icoSphere.addToVertices(&triangleVertices, centre,
		                                          Ogre::ColourValue(colour.r, colour.g, colour.b, fillAlpha), radius);
